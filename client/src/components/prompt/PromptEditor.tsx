...
export default function PromptEditor({
  ...
  useEffect(() => {
    if (promptData) {
      form.reset({
        ...
        isPublic:
          typeof promptData.isPublic === "boolean" ? promptData.isPublic : true,
      });
  ...
  if (promptLoading) {
    return (
      <div className="p-8 flex justify-center">Loading prompt data...</div>
    );
  }
              ...
              <div className="border border-accent rounded-lg overflow-hidden focus-within:ring-1 focus-within:ring-primary focus-within:border-primary">
                <div className="bg-[#F9F9FC] border-b border-accent px-3 py-1.5 flex items-center space-x-1.5">
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="p-1.5 hover:bg-accent rounded text-gray-700"
                  >
                ...
                {aiSuggestions.length > 0 ? (
                  <div className="space-y-2">
                    {aiSuggestions.map((suggestion, i) => (
                      <div
                        key={i}
                        className="text-sm text-gray-700 bg-white p-2 rounded border border-gray-200 flex items-center justify-between"
                      >
                        <p>{suggestion}</p>
                        <Button
                          type="button"
